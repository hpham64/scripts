...
# Debian 12 LTS System Configuration Tweaks

OS=$(cat /etc/os-release | grep -w VERSION_ID)

if [ "$OS" != 'VERSION_ID="12"' ] ; then
        echo "OS is not Debian 12"
        exit
fi
...
if [ "$interpreter" != "bash" ]; then
        echo "Please run with bash. (\`./debian12-preconfig.sh\` or \`bash debian12-preconfig.sh\`)"
...
|_____  ##|_____  ##| ##  | ##| ##  \__/| ## \  ##/##/| ########|  ######
      ...
      |__/ \______/ |_______/ |__/      |__/    \_/    \_______/|_______/
                                           ...
                                           Debian Preconfiguration Script.
...
add_cockpit() {
    ...
    #Add contrib entry for zfs installation
    ...
    if [[ $res != 0 ]]; then
        ...
        apt install -y cockpit cockpit-benchmark cockpit-navigator cockpit-file-sharing cockpit-45drives-hardware cockpit-identities  \
                cockpit-sosreport realmd tuned udisks2-lvm2 zfs-dkms samba winbind nfs-kernel-server nfs-common 45drives-tools cockpit-scheduler cockpit-zfs nano tuned 
...
if [[ -f .debian12-preconfig.progress ]]; then
        progress=$(cat .debian12-preconfig.progress)
fi

if [[ $progress != "" ]]; then
        echo "Found progress from previous time running this script. ($PWD/.debian12-preconfig.progress)"
